...
import { DelphinusContract, DelphinusReadOnlyProvider } from "./client";
import { DBHelper, withDBHelper } from "./dbhelper";
import { EventLog, JsonRpcProvider, Log } from "ethers";
...
export class EventTracker {
  private readonly provider: DelphinusReadOnlyProvider;
  ...
  constructor(
    ...
    this.provider = new DelphinusReadOnlyProvider(providerUrl);
...
async function getLatestBlockNumberFromSource(providerUrl: string) {
  let provider = GetProvider(providerUrl);
  try {
